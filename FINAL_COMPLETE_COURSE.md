...
# SUPPLEMENT — Missing & Expanded Sections
## Additions to the AI Certification Bootcamp
### Zero coding background assumed · Every concept explained first

---

> **How to use this file:**
> This supplements COMPLETE_COURSE.md.
> Read the relevant COMPLETE_COURSE week first, then read the matching section here.
> Every concept starts from scratch — no assumptions about what you already know.

---

## Table of Contents

**CCA Gaps (Claude Certified Architect)**
- [S-CCA-1: The Claude Agent SDK — What It Is and How to Use It](#s-cca-1-the-claude-agent-sdk)
- [S-CCA-2: Claude Code — Plan Mode, /init, Custom Agent Skills](#s-cca-2-claude-code-advanced-topics)
- [S-CCA-3: Confidence Calibration and Provenance Tracking](#s-cca-3-confidence-calibration-and-provenance)
- [S-CCA-4: Expanded Anti-Patterns with Full Context](#s-cca-4-expanded-anti-patterns)

**NCA-AIIO Gaps (NVIDIA AI Infrastructure)**
- [S-NV-1: DGX Systems — Power, Cooling, and Data Centre Planning](#s-nv-1-dgx-and-data-centre-planning)
- [S-NV-2: Base Command Manager — NVIDIA Cluster Management](#s-nv-2-base-command-manager)
- [S-NV-3: NVIDIA Nsight — Profiling and Debugging](#s-nv-3-nvidia-nsight-profiling)
- [S-NV-4: GPUDirect Storage and RoCE — Expanded](#s-nv-4-gpudirect-storage-and-roce)
- [S-NV-5: vGPU — Virtual GPU Technology](#s-nv-5-vgpu)

---

# S-CCA-1: The Claude Agent SDK

## What Is the Agent SDK — Plain English

So far in this course, every agent loop has been written manually in Python:
- You write the while loop
- You check `stop_reason` yourself
- You call tools yourself
- You manage the message history yourself

The **Claude Agent SDK** is a higher-level toolkit that does all of that boilerplate for you. Think of it as a ready-made agent loop that you configure rather than build from scratch.

> 🏗️ **Construction analogy:** Building an agent from raw API calls is like building a house by mixing your own cement, cutting your own lumber, and wiring your own electricity. The Agent SDK is like buying a prefabricated house kit — the structure is already built. You customise it for your specific needs.

⚡ **Why the CCA exam tests this:** The exam distinguishes between knowing the raw API (which you have learned) and knowing the Agent SDK patterns. Questions describe a scenario and ask which approach — raw API or Agent SDK — is appropriate, and how to configure the SDK correctly.

---

## Before We Write Code — Understanding What a Library Does

If you have never used a Python library before, here is the mental model:

A library is code someone else wrote that you can use in your own program. Instead of writing everything from scratch, you import their code and call their functions.

```python
# This line says: "bring in the anthropic library so I can use it"
import anthropic

# This line says: "use the Anthropic class from that library to create a client object"
client = anthropic.Anthropic()
```

The Agent SDK works the same way — it is a library that provides pre-built agent behaviour.
...
```bash
# The Agent SDK is part of the anthropic library
# If you installed it in Week 1, you already have it
pip install anthropic

# Verify the version (need 0.40.0 or higher for Agent SDK features)
python -c "import anthropic; print(anthropic.__version__)"
```

---

## The Core Concept — AgentLoop

The Agent SDK's central idea is that you define **what tools the agent has** and **what happens when each tool is called**. The SDK handles the loop, the message history, and the stop conditions automatically.

### Step 1 — Understand What We Are Building

Without the SDK, an agent looks like this:

```python
# RAW API — you manage everything manually
messages = [{"role": "user", "content": goal}]
while True:
...
```python
# AGENT SDK — the SDK manages the loop
# You just define tools and the SDK does the rest
```
...
Let us build this step by step, explaining every piece.

```python
# save as: sdk_agent_step1.py
# pip install anthropic

import anthropic

# Step 1: Create the client (same as always)
client = anthropic.Anthropic()

# Step 2: Define what your agent can do (tools)
# Each tool is a Python function decorated with @tool
# The function's docstring becomes the tool description Claude reads

# We will build a simple helper agent that can:
# - look up a product price
# - calculate discounts
# - check stock

# First, define the data our "fake database" has
PRODUCTS = {
    "laptop":   {"price": 999,  "stock": 15},
    "monitor":  {"price": 299,  "stock": 42},
    "keyboard": {"price": 79,   "stock": 200},
    "mouse":    {"price": 29,   "stock": 300},
    "headset":  {"price": 149,  "stock": 8},
}

# Define tool functions
# The name and description must be clear — Claude reads them to decide when to call each function

def get_product_info(product_name: str) -> dict:
    """
    Get the price and stock level for a product.
    Use when the user asks about a product's price or availability.
    Input: product name as a lowercase string (e.g. 'laptop', 'keyboard').
    Returns: dict with price (USD) and stock count, or error if not found.
    """
...
def calculate_discount(original_price: float, discount_percent: float) -> dict:
    ...
    Use when the user asks about discounts, sale prices, or savings.
    Input: original_price (number), discount_percent (0-100).
    Returns: final_price, amount_saved, discount_percent.
    """
    if discount_percent < 0 or discount_percent > 100:
        return {"error": "Discount must be between 0 and 100 percent"}

    amount_saved = original_price * (discount_percent / 100)
    final_price  = original_price - amount_saved
...
def check_multiple_products(product_names: list) -> dict:
    """
    Check prices and stock for multiple products at once.
    Use when the user asks about several products in one question.
    Input: list of product name strings.
    Returns: dict with results for each product.
    """
    results = {}
    for name in product_names:
        results[name] = get_product_info(name)
    return results
```

Now let us wire these into an agent using the SDK pattern:

```python
# save as: sdk_agent_step2.py
import anthropic
import json
...
def get_product_info(product_name: str) -> dict:
    ...
    if name in PRODUCTS:
        p = PRODUCTS[name]
        return {"found": True, "product": name, "price": p["price"],
                "stock": p["stock"], "in_stock": p["stock"] > 0}
    return {"found": False, "error": f"'{product_name}' not found",
            "available": list(PRODUCTS.keys())}

def calculate_discount(original_price: float, discount_percent: float) -> dict:
    if not (0 <= discount_percent <= 100):
        return {"error": "Discount must be 0-100"}
    saved = original_price * (discount_percent / 100)
    return {
        "original_price": round(original_price, 2),
        "amount_saved":   round(saved, 2),
        "final_price":    round(original_price - saved, 2)
    }

def check_multiple_products(product_names: list) -> dict:
    return {name: get_product_info(name) for name in product_names}

# --- SDK-style tool definitions ---
# This is the format the Claude API uses to describe tools
TOOLS = [
    {
        "name": "get_product_info",
        "description": "Get price and stock level for a single product. Use when asked about one specific product's price or availability. Input: product name string. Returns: price in USD and stock count.",
        "input_schema": {
            "type": "object",
            "properties": {
                "product_name": {
                    "type": "string",
                    "description": "Product name in lowercase, e.g. 'laptop', 'keyboard'"
                }
            },
    ...
    },
    {
        ...
        "description": "Calculate the final price after a percentage discount. Use when asked about discounts, sale prices, or how much would be saved.",
        "input_schema": {
            "type": "object",
            "properties": {
                "original_price": {
    ...
    {
        "name": "check_multiple_products",
        "description": "Check prices and stock for several products at once. Use when asked about multiple products in one question. More efficient than calling get_product_info multiple times.",
        "input_schema": {
            "type": "object",
            "properties": {
                "product_names": {
...
# --- Tool dispatcher ---
# This function receives the tool name and inputs from Claude
# and calls the right Python function
def run_tool(tool_name: str, tool_input: dict) -> str:
    """
    Run a tool and return the result as a string.
    Claude receives this string as the tool result.
    """
    try:
        if tool_name == "get_product_info":
            result = get_product_info(tool_input["product_name"])

        elif tool_name == "calculate_discount":
            result = calculate_discount(
                tool_input["original_price"],
                tool_input["discount_percent"]
            )

        elif tool_name == "check_multiple_products":
            result = check_multiple_products(tool_input["product_names"])

        else:
            result = {"error": f"Unknown tool: {tool_name}"}

    except KeyError as e:
        result = {"error": f"Missing required input: {e}"}
    except Exception as e:
        result = {"error": f"Tool error: {str(e)}"}

    # Always return a string — Claude reads strings as tool results
...
def run_agent(user_question: str) -> str:
    ...
    This function implements the 'Agent SDK pattern':
    - Manages the conversation history
    - Runs the tool loop automatically
    - Enforces stopping conditions
    - Returns the final answer

    Think of this as what the Agent SDK does for you in production.
    """
    ...
    MAX_ITERATIONS = 10    # Hard ceiling — no runaway loops
    error_count    = 0     # Track failures
    MAX_ERRORS     = 3     # Stop if too many tools fail
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        response = client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=1024,
            system="""You are a helpful shopping assistant for an electronics store.
            Help customers with product information, pricing, and stock availability.
            Always be accurate about prices — use the tools to look up real prices.
            If a customer asks about multiple products, use check_multiple_products for efficiency.""",
            tools=TOOLS,
            messages=messages
        )
        ...
        # --- STOPPING CONDITION 1: SUCCESS ---
        # Claude decided it has enough information and wrote a final answer
        if response.stop_reason == "end_turn":
        ...
        # --- TOOL USE: Claude wants to call a function ---
        if response.stop_reason == "tool_use":

            # Add Claude's response (including its tool call requests) to history
            messages.append({
                "role": "assistant",
                "content": response.content
            })

            # Process every tool call Claude made in this response
            ...
            for block in response.content:
                if block.type == "tool_use":
                    ...
                    print(f"  With inputs: {json.dumps(block.input)}")
            ...
            # --- STOPPING CONDITION 2: TOO MANY ERRORS ---
            if error_count >= MAX_ERRORS:
                return f"I encountered too many errors ({error_count}) while looking up your request. Please try again or contact support."
    ...
    # --- STOPPING CONDITION 3: MAX ITERATIONS REACHED ---
    return f"I was unable to complete this request within {MAX_ITERATIONS} steps. Please try a simpler question."


# --- TEST THE AGENT ---

# Simple question — one tool call
print(run_agent("How much does a laptop cost?"))

# Multi-step question — needs price lookup then calculation
print(run_agent("If I get a 15% discount on the headset, how much do I save?"))

# Multiple products — should use check_multiple_products efficiently
print(run_agent("What are the prices for keyboard, mouse, and headset? Which is cheapest?"))

# Out of scope
print(run_agent("Do you have any smartphones in stock?"))
```

---

## The Agent SDK — What It Adds on Top

The pattern above IS the Agent SDK pattern. In production code with the official SDK helpers, you get these additions:

### 1 — Automatic Message History Management

```python
# sdk_agent_step3.py
# The Agent SDK manages conversation history automatically

import anthropic

client = anthropic.Anthropic()

# Instead of manually building message arrays,
# the SDK tracks history for you

# This is what the SDK's internal loop looks like conceptually:
class AgentSession:
    """
    Simplified version of what the Agent SDK provides.
    The real SDK has more features but this captures the core pattern.
    """

    def __init__(self, system_prompt: str, tools: list, tool_runner):
    ...
    def run(self, user_message: str, max_iterations: int = 10) -> str:
        ...
        for iteration in range(max_iterations):
            ...
            if response.stop_reason == "tool_use":
                ...
                for block in response.content:
                    if block.type == "tool_use":
                        result = self.tool_runner(block.name, block.input)
                        tool_results.append({
                            "type":        "tool_result",
                            "tool_use_id": block.id,
                            "content":     result
...
## When to Use Agent SDK vs Raw API — Exam Decision Table

⚡ **The CCA exam asks this directly.**

| Scenario | Use | Why |
|----------|-----|-----|
| Simple one-off tool call | Raw API | Overhead of SDK not needed |
| Agent that runs many iterations | Agent SDK | Manages loop, history, errors automatically |
| Production support bot | Agent SDK | Built-in reliability patterns |
| Extracting structured data | Raw API with forced tool_choice | Not an agent — just one structured call |
| Research agent with 10+ tool calls | Agent SDK | Complex lifecycle management |
| Testing and prototyping | Raw API | Simpler, easier to debug |
| Team building many agents | Agent SDK | Consistent patterns, less boilerplate |

---

## S-CCA-1 Flashcards

**Q: What does the Claude Agent SDK provide that the raw API does not?**
A: Automatic management of the agent loop, message history, tool dispatch, stopping conditions, and error handling. Reduces boilerplate and enforces consistent patterns.

**Q: What are the three stopping conditions the Agent SDK enforces?**
A: (1) `end_turn` — Claude finished successfully. (2) Error threshold — too many tool failures. (3) Max iterations — hard ceiling to prevent runaway loops.

**Q: When would you use the raw API instead of the Agent SDK?**
A: For simple single-call tasks like structured data extraction (no loop needed), for quick prototyping, or for tasks that only need one or two tool calls.

**Q: In the Agent SDK pattern, what is the tool dispatcher?**
A: A function that receives the tool name and inputs from Claude, calls the right Python function, and returns the result as a string. It is the bridge between Claude's tool requests and your actual code.

**Q: Why must tool results always be returned as strings?**
A: The Claude API expects `tool_result` content as a string. JSON-encode the result with `json.dumps()` to convert dicts, lists, and numbers to string format that Claude can read.

---

---

# S-CCA-2: Claude Code — Advanced Topics

## Before We Start — What Claude Code Is

Claude Code is a program that runs in your terminal (the black command-line window). It reads your code files, writes changes, and takes multi-step actions inside your actual project folder.

If you have never used a terminal before, here is the minimal knowledge you need:

```bash
# A terminal lets you type commands and press Enter to run them
# Here are the only commands you need for Claude Code:

cd folder-name      # Change into a folder (cd = change directory)
...
Claude Code works by you typing instructions in plain English, and it does the work inside whatever folder you are currently in.
...
Imagine asking a contractor to renovate your kitchen. Would you rather:
- Option A: They immediately start knocking down walls and rewiring electricity
- Option B: They first show you a detailed plan of exactly what they will do, you approve it, then they start

Plan mode is Option B for Claude Code.

### What Plan Mode Does

When you run Claude Code with the `--plan` flag, Claude:
1. Reads your request
2. Figures out exactly what changes would be needed
3. Shows you a detailed plan: which files it would change, what it would do to each
4. **Stops before making any changes**
5. Waits for you to say "yes, proceed" or "no, cancel"

```bash
# WITHOUT plan mode — Claude immediately starts making changes
claude "Add input validation to all API endpoints"

# WITH plan mode — Claude shows you what it would do first
claude --plan "Add input validation to all API endpoints"

# Claude output in plan mode:
# PLAN:
# 1. Read: src/routes/users.py (understand current structure)
# 2. Read: src/routes/products.py
# 3. Modify: src/routes/users.py — add email format validation, required field checks
# 4. Modify: src/routes/products.py — add price range validation, name length check
# 5. Create: src/validators.py — shared validation helper functions
# 6. Modify: tests/test_routes.py — add test cases for validation
#
# Proceed? (y/n):
```
...
⚡ **CCA Domain 2 — plan mode decision is exam-tested.**

| Use plan mode (`--plan`) | Use direct mode (no flag) |
|--------------------------|--------------------------|
| Making changes to many files | Quick single-file fix |
| Anything touching production code | Low-stakes exploration |
| You are not 100% sure what Claude will do | You trust the task is simple |
| Reviewing changes before your team sees them | Local experiment |
| Any destructive operation (delete, rename) | Adding new files only |
| First time doing a new type of task | Repeated routine task |

```bash
# Examples where you ALWAYS use --plan:
claude --plan "Refactor the authentication module"
claude --plan "Delete all temporary files and unused imports"
claude --plan "Update the database schema"
claude --plan "Rename the User class to Account everywhere"

# Examples where direct mode is fine:
claude "Add a comment explaining this function"
claude "Fix the typo in the error message on line 47"
claude "Show me what this function does"
```

### Auto-Accept Mode

The opposite of plan mode. When you are confident and want Claude to proceed without asking for confirmation at each step:

```bash
# Claude proceeds automatically without asking "should I do this?"
claude --yes "Run the test suite and fix any failing tests"

# Use with caution — only for trusted, well-understood tasks
```
...
`/init` is a slash command you type inside a Claude Code session. It analyses your current repository and automatically generates a `CLAUDE.md` file tailored to your specific project.
...
# Claude Code will:
# 1. Read your folder structure
# 2. Identify your programming language(s)
# 3. Find your package manager (npm, pip, etc.)
# 4. Detect your test framework
# 5. Identify existing conventions in your code
# 6. Generate a CLAUDE.md with all of this information
```
...
Auto-generated by /init on 2026-03-15
...
- API endpoints follow REST conventions (/api/v1/resource)
- All database operations go through the service layer, not directly in routes

## Commands (detected from Makefile)
- Run tests:    make test
- Start server: make run
- Lint:         make lint

## Rules Claude detected
- Environment variables loaded from .env file (never hardcode)
- Logging uses structlog (never print())
```

### After /init — Customise the Generated File

`/init` gives you a starting point. You should edit it to add your team's specific rules:

```markdown
## AFTER /init — add these manually:
...
## Rules — ALWAYS
- New API endpoints must have rate limiting
- Database queries must go through the service layer
...
Custom Agent Skills are reusable instruction files that Claude Code automatically applies when certain conditions are met. Think of them as shortcuts for tasks your team does repeatedly.

> 📋 **Standard operating procedure analogy:** In a company, some tasks are done so often that you create a Standard Operating Procedure (SOP) document. Every time someone needs to do that task, they follow the SOP. Custom Agent Skills are SOPs for Claude Code — write the instructions once, Claude Code follows them automatically every time that type of task comes up.

### Creating a Custom Skill

```bash
# Skills live in a special folder in your project
mkdir -p .claude/skills

# Create a skill file
# The filename describes when it triggers
# The content tells Claude what to do
```
...
When the user asks to create, add, or build a new API endpoint or route.

## Steps to follow
1. Ask for: endpoint path, HTTP method, request body schema, response schema
2. Create the route handler in src/routes/{resource}.py
3. Create the service function in src/services/{resource}.py
4. Add input validation using our standard validator pattern
5. Add the route to src/main.py router registration
6. Write unit tests in tests/test_{resource}.py
7. Update API documentation in docs/api.md

## Our conventions to follow
- Route files: snake_case, plural noun (users.py, products.py)
- Service functions: verb_noun format (get_user, create_order)
- Always add rate limiting decorator: @rate_limit(calls=100, period=60)
- Error responses must use our standard format: {"error": str, "code": str}

## Do not do
- Do not put business logic in route handlers
- Do not access the database directly from routes
- Do not forget to add error handling for database failures
```
...
## When this skill applies
When the user asks to write, add, or create tests for existing code.

## Steps to follow
1. Read the code being tested to understand what it does
2. Identify: happy path, edge cases, error cases
3. Write tests in the matching tests/ directory
4. Use pytest fixtures for database setup/teardown
5. Mock external API calls using unittest.mock
6. Ensure each test has a clear docstring explaining what it tests
7. Run the tests with: pytest tests/test_{module}.py -v
...
Claude Code reads your skill files at the start of each session. When your request matches the "when this skill applies" section, Claude Code automatically follows the skill's steps.

```bash
# This request automatically triggers create-api-endpoint.md
claude "Add an endpoint to get user order history"

# This triggers write-tests.md
claude "Write tests for the new payment service"

# This triggers nothing special — Claude just helps normally
claude "Explain what this function does"
```

### Distributing Skills to Your Team
...
# Now every team member who clones the repo
# automatically gets all the team's skills
```

⚡ **CCA exam point:** Skills are stored in `.claude/skills/` directory. They are committed to the repository so all team members share them. They trigger automatically based on context matching.

---

## S-CCA-2 Flashcards

**Q: What does `claude --plan` do that `claude` alone does not?**
A: Plan mode shows Claude's intended actions (which files, what changes) and waits for approval before making any changes. Direct mode starts executing immediately.

**Q: When should you ALWAYS use plan mode?**
A: When making changes across many files, any destructive operations (delete, rename), changes to production code, or any time you are uncertain what Claude will do.

**Q: What does the `/init` command do?**
A: Analyses your repository and auto-generates a CLAUDE.md file containing your detected tech stack, project structure, conventions, and commands.

**Q: What is a Custom Agent Skill?**
A: A reusable markdown instruction file in `.claude/skills/` that Claude Code automatically applies when the task context matches the skill's trigger description.

**Q: Where are Custom Agent Skills stored and how are they shared?**
A: In `.claude/skills/` within the repository. Committed to Git so all team members automatically get the same skills when they clone or pull.

---

---

# S-CCA-3: Confidence Calibration and Provenance

## Confidence Calibration — What It Means

### Plain English First

When you ask a person a question, a trustworthy person will say "I am certain about this" for things they know well, and "I think this might be the case, but you should verify it" for things they are less sure about.

**Confidence calibration** is designing your Claude system so it communicates its certainty level in a structured, usable way — not just as words, but as a number or category your code can act on.

> ⚖️ **Weather forecast analogy:** A weather app does not just say "it might rain." It says "70% chance of rain." That number is actionable — you know whether to bring an umbrella. Confidence calibration gives Claude's uncertainty a number your system can use to decide whether to answer, escalate, or ask for more information.

### Why It Matters for Architecture

⚡ **CCA Domain 1 and 5** — confidence calibration determines escalation logic.

Without confidence calibration:
```
User asks question → Claude answers → User trusts it → Maybe wrong
```

With confidence calibration:
```
User asks question
    → Claude answers AND provides confidence score
    → If score > 80%: return answer to user
    → If score 50-80%: return answer with "verify this" caveat
    → If score < 50%: escalate to human agent
```

### Implementing Confidence Calibration

```python
# save as: confidence_calibration.py
import anthropic, json

client = anthropic.Anthropic()

# Define a tool that forces Claude to express its confidence
confident_answer_tool = {
    "name": "provide_answer",
    "description": "Provide an answer with an explicit confidence level and reasoning.",
    "input_schema": {
        "type": "object",
        "properties": {
            "answer": {
            ...
            "confidence_score": {
                ...
                "minimum": 0,
                "maximum": 100,
                "description": "Your confidence in this answer, 0-100. Be honest. 90+ = very certain. 70-89 = fairly confident. 50-69 = uncertain. Below 50 = should escalate."
            },
            "confidence_reason": {
                "type": "string",
                "description": "Why you have this confidence level — what makes you certain or uncertain"
            },
            "sources_needed": {
                "type": "boolean",
                "description": "True if a human should verify this with authoritative sources"
            },
            "escalate_to_human": {
                "type": "boolean",
                "description": "True if a human expert should handle this question"
            }
        },
        "required": ["answer", "confidence_score", "confidence_reason",
                     "sources_needed", "escalate_to_human"]
    }
}

def ask_with_confidence(question: str, domain: str = "general") -> dict:
    """
    Ask a question and get a response with an explicit confidence level.
    Returns the full structured response so your system can act on the confidence.
    """
    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=512,
        system=f"""You are a knowledgeable assistant in the domain of: {domain}.
        When answering questions:
        - Be honest about your confidence level
        - Score below 70 if you are not certain
        - Score below 50 if the question requires real-time data, specific expertise, or involves someone's health/safety/legal situation
        - Always say if a human expert should verify or handle the question""",
        tools=[confident_answer_tool],
...
def handle_response(question: str, domain: str = "general") -> str:
    """
    Handle a question with automatic escalation based on confidence.
    """
    result = ask_with_confidence(question, domain)
    ...
    print(f"\nQuestion: {question}")
    print(f"Confidence: {confidence}/100 — {result['confidence_reason']}")

    # Route based on confidence level
    if escalate or confidence < 50:
        print(f"Action: ESCALATE TO HUMAN — confidence too low or expert needed")
        return f"[Escalating to human agent] {answer}"

    elif confidence < 70:
        print(f"Action: ANSWER WITH CAVEAT")
        return f"{answer}\n\n⚠️ Note: I am not fully certain about this. Please verify with an authoritative source."

    elif result["sources_needed"]:
        print(f"Action: ANSWER WITH SOURCE RECOMMENDATION")
        return f"{answer}\n\n📚 Recommendation: Verify this with official documentation."

    else:
        print(f"Action: CONFIDENT ANSWER")
        return answer

# Test with different confidence scenarios
questions = [
    ("What is Python?",                               "programming"),
    ("What is the current price of NVIDIA stock?",    "finance"),
    ("Should I take ibuprofen with my blood pressure medication?", "medical"),
    ("What does the HTTP 429 status code mean?",      "web development"),
    ("Who will win the next election?",               "politics"),
]

for question, domain in questions:
    response = handle_response(question, domain)
    print(f"Response: {response[:100]}...\n")
```

---

## Provenance Tracking — What It Means

### Plain English First

**Provenance** means "where did this come from?" In art, provenance is the history of who owned a painting. In AI systems, provenance tracking means recording *which agent, which tool, which data source* produced each piece of information in the final output.

> 🔍 **Medical record analogy:** When a doctor reviews your test results, each result has provenance: which lab ran it, on what date, using which equipment. If a result looks wrong, they know exactly who to call. Provenance tracking in multi-agent systems gives you the same traceability.

### Why It Matters

In a multi-agent research system:
```
Orchestrator asks 3 sub-agents for research
    → Sub-agent A searches Product database
    → Sub-agent B searches Support tickets
    → Sub-agent C searches Documentation
    → Orchestrator synthesises into a report

Problem: The final report says "users reported 47 crashes last month"
         Which agent said this? Was it Sub-agent B from support tickets?
         Or was it Sub-agent A from the product database?
         If it is wrong, who do you investigate?
```

Without provenance tracking, you cannot trace errors back to their source.

### Implementing Provenance Tracking

```python
# save as: provenance_tracking.py
import anthropic, json
from datetime import datetime

client = anthropic.Anthropic()

# --- Data structures for tracking provenance ---

def create_sourced_finding(
    content: str,
    source_agent: str,
    source_tool: str,
    confidence: int,
    ...
    """
    Create a finding with full provenance information.
    Every piece of information your system produces should have this.
    """
    return {
        "content":      content,
        "provenance": {
            "source_agent": source_agent,    # which agent produced this
            "source_tool":  source_tool,     # which tool was called
            "timestamp":    datetime.now().isoformat(),
            "confidence":   confidence,      # 0-100
            "raw_data":     raw_data[:200]   # trimmed source data
        }
    }

# --- Simulated data sources ---
PRODUCT_DB = {
    "crashes": {"count": 47, "period": "last_30_days", "severity": "P2"},
    "users":   {"active": 12500, "new_this_month": 340}
}

SUPPORT_TICKETS = [
    {"id": "TKT-001", "issue": "app crash on login", "count": 23},
    {"id": "TKT-002", "issue": "crash during export", "count": 24},
]

DOCUMENTATION = {
    "known_issues": ["Login crash fixed in v2.3", "Export issue under investigation"],
    "version":      "2.2.1"
}

# --- Sub-agent that tracks provenance ---
def run_research_subagent(
    ...
    """
    Run a research sub-agent that returns findings with provenance.
    """

    # The tool forces structured output with provenance
    research_tool = {
        "name": "report_findings",
        "description": "Report research findings with source information",
        "input_schema": {
            "type": "object",
            "properties": {
                "findings": {
                    ...
                    "items": {
                        ...
                        "required": ["finding", "confidence", "data_points"]
                    }
                }
            },
            "required": ["findings"]
    ...
    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=512,
        system=f"You are {agent_name}. Analyse the provided data and report findings. Be specific and cite data points.",
        tools=[research_tool],
    ...
    raw_findings = response.content[0].input["findings"]

    # Attach provenance to each finding
    sourced_findings = []
    for finding in raw_findings:
        sourced_findings.append(
            create_sourced_finding(
                content=      finding["finding"],
                source_agent= agent_name,
                source_tool=  "report_findings",
                confidence=   finding["confidence"],
                raw_data=     str(finding.get("data_points", []))
            )
        )

    return sourced_findings

def synthesise_with_provenance(
    all_findings: list[dict],
    report_topic: str
) -> str:
    """
    Synthesise findings into a report, preserving provenance references.
    """

    # Format findings for the synthesis step
    formatted = []
    for i, f in enumerate(all_findings):
        prov = f["provenance"]
        formatted.append(
            f"[Finding {i+1}] Source: {prov['source_agent']} | Confidence: {prov['confidence']}/100\n"
            f"Content: {f['content']}"
        )
    ...
    response = client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=1024,
        system="""You synthesise research findings into a clear report.
        For each claim in your report, cite the finding number in square brackets, e.g. [Finding 1].
        This allows readers to trace every claim back to its source.""",
        messages=[{
            "role": "user",
            "content": f"Topic: {report_topic}\n\nFindings:\n{findings_text}\n\nWrite a concise report citing sources."
        }]
    )

    return response.content[0].text

# --- Run the full provenance-tracked pipeline ---
print("Running multi-agent research with provenance tracking...\n")

# Sub-agent 1: Product database analysis
product_findings = run_research_subagent(
    agent_name="ProductDBAgent",
    task="Analyse crash data and user metrics",
    data_source=PRODUCT_DB
)
print(f"ProductDBAgent found {len(product_findings)} findings")

# Sub-agent 2: Support ticket analysis
support_findings = run_research_subagent(
    agent_name="SupportTicketAgent",
    task="Analyse recent crash-related support tickets",
    data_source={"tickets": SUPPORT_TICKETS}
)
print(f"SupportTicketAgent found {len(support_findings)} findings")

# Sub-agent 3: Documentation review
doc_findings = run_research_subagent(
    agent_name="DocumentationAgent",
    task="Review known issues and current software version",
    data_source=DOCUMENTATION
)
print(f"DocumentationAgent found {len(doc_findings)} findings")

# Combine all findings
all_findings = product_findings + support_findings + doc_findings

# Generate report with provenance
report = synthesise_with_provenance(
    all_findings=all_findings,
    report_topic="Application Stability Analysis — Last 30 Days"
)

print("\n" + "="*55)
print("FINAL REPORT (with provenance citations):")
print("="*55)
print(report)
...
for i, f in enumerate(all_findings):
    p = f["provenance"]
    print(f"[Finding {i+1}] {p['source_agent']} | confidence: {p['confidence']}/100 | {p['timestamp'][:19]}")
...
**Q: What is confidence calibration in Claude systems?**
A: Designing the system so Claude explicitly reports its certainty level (typically 0–100) so that the application can decide whether to return the answer, add caveats, or escalate to a human.

**Q: At what confidence level should an agent typically escalate to a human?**
A: Below 50% is a common threshold for escalation. 50–70% usually warrants a caveat ("please verify this"). Above 80% can typically be returned directly. These thresholds should be tuned to your specific use case and risk tolerance.

**Q: What is provenance tracking?**
A: Recording which agent, which tool, and which data source produced each piece of information in a multi-agent system's output — so errors can be traced back to their source.

**Q: Why is provenance especially important in multi-agent systems?**
A: When multiple sub-agents contribute findings to a synthesised report, it becomes impossible to trace a wrong claim without provenance metadata. With provenance, every claim cites its source agent, tool, timestamp, and confidence level.

---

---

# S-NV-1: DGX Systems — Power, Cooling, and Data Centre Planning

## Why Data Centre Planning Is on the Exam

The NCA-AIIO tests whether you understand the *operational reality* of deploying AI hardware. It is not enough to know that H100 GPUs are powerful — you must know what happens when you actually put them in a building. Power and cooling are the most common real-world blockers to AI deployment.

---

## DGX Systems — Complete Reference

### DGX H100

NVIDIA's current flagship AI system.

```
DGX H100 — Full Specifications
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GPUs:            8× NVIDIA H100 SXM 80GB
Total GPU RAM:   640GB HBM3
GPU-to-GPU:      NVLink 4.0 + NVSwitch (full mesh, 900 GB/s per GPU)
CPU:             2× Intel Xeon Platinum (56 cores total)
System RAM:      2TB DDR5
Storage:         30TB NVMe SSD (internal)
Networking:      8× 400Gb/s InfiniBand (ConnectX-7)
Power:           10.2kW maximum draw
Form factor:     6U (takes 6 rack units of height in a rack)
Weight:          ~130kg (approximately 285 lbs)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
DGX A100 — Key Specs
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GPUs:            8× NVIDIA A100 SXM 80GB
Total GPU RAM:   640GB HBM2e
GPU-to-GPU:      NVLink 3.0 + NVSwitch (600 GB/s per GPU)
Power:           6.5kW maximum draw
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

### DGX B200 (Blackwell)

Newest generation, being deployed in 2025–2026.

```
DGX B200 — Key Specs
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
GPUs:            8× NVIDIA B200 SXM 192GB
Total GPU RAM:   1.5TB HBM3e
Power:           14.3kW maximum draw
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
DGX Station H100
━━━━━━━━━━━━━━━━━
GPUs:  4× H100 80GB (desktop form factor)
Power: 1.5kW (plugs into standard power outlet)
```

---

## Power — The Hidden Deployment Blocker

### Why Power Is The #1 Problem

Most companies trying to deploy AI discover their existing data centres cannot support the power density AI requires.

```
Standard server:    200–400W per 1U server
AI GPU server:      3,000–10,000W per 2U server
DGX H100:           10,200W per 6U system

Standard data centre rack: can handle 5–15kW per rack
AI data centre rack:       needs 30–100kW per rack
```

### Power Calculation Example

⚡ **The exam presents calculation scenarios like this.**

```
Scenario: Deploy 4 DGX H100 systems in one rack

Each DGX H100: 10.2kW
4 systems:     4 × 10.2 = 40.8kW per rack

Standard data centre rack budget: 10–15kW
AI rack budget needed:            40.8kW

= This rack requires purpose-built AI data centre infrastructure
= Standard colocation will not work
```

### Power Infrastructure Requirements

| Component | Standard DC | AI Data Centre |
|-----------|-------------|----------------|
| Power per rack | 5–15 kW | 30–100 kW |
| Power redundancy | N+1 | 2N (full redundancy) |
| UPS capacity | Standard | Oversized for AI loads |
| PDU type | Standard | High-density, smart PDUs |
| Power factor | ~0.9 | Requires power factor correction |

---

## Cooling — Keeping AI Hardware Running

### Why Cooling Is Critical

An H100 GPU at full load generates 700W of heat. An entire DGX H100 with 8 H100s plus CPUs generates over 10kW — roughly equivalent to 100 hair dryers running simultaneously. All of that heat must be removed from the data centre.

If cooling fails, thermal throttling begins and then GPUs shut down automatically to prevent damage.

### Three Cooling Methods

#### Method 1 — Air Cooling (Traditional)

```
How it works:
  Cold air enters from the front of the rack
  Fans push it through the servers
  Hot air exits from the back
  Air conditioning units cool and recirculate the air

Maximum density: ~20kW per rack
Cost: Lowest
Complexity: Simple
Limitation: Not enough for high-density AI
```
...
```
How it works:
  Cold water pipes run directly to the GPU cooling plates
  Water absorbs heat directly from the chip
  Hot water goes to a heat exchanger outside the rack
  No reliance on cold air

Maximum density: 100kW per rack
Cost: Higher (requires plumbing)
Complexity: Medium — requires water infrastructure
Required for: DGX H100, DGX B200 at full performance
```

DLC is what NVIDIA recommends for DGX H100 and newer systems.

#### Method 3 — Immersion Cooling

```
How it works:
  Servers are submerged in tanks of non-conductive liquid
  Liquid absorbs heat directly from all components
  No fans required
  Liquid pumped to external heat exchanger

Maximum density: 200kW+ per tank
Cost: Highest
Complexity: High
Advantage: Best performance, lowest noise, highest density
Emerging use: Hyperscale AI data centres
```

### Exam Scenario — Cooling Selection

⚡ **NCA-AIIO scenario type:**

```
Q: A company is deploying 8 DGX H100 systems. Their existing
   data centre uses air cooling rated at 15kW per rack.
   What problem do they face and what is the solution?

A: Each DGX H100 draws 10.2kW. A full rack of DGX H100s
   (which can hold only 2 systems at 6U each) would draw
   ~20.4kW — already exceeding the 15kW air cooling capacity.
   
   Solution: Upgrade to Direct Liquid Cooling (DLC).
   DLC can handle 100kW per rack, sufficient for 2 DGX H100s
   with significant headroom.
```

---

## Data Centre Redundancy for AI

### N+1 Redundancy

For every critical component, one spare is available:
- If you need 4 power circuits, you have 5
- If one fails, the 4 remaining handle the load

### 2N Redundancy (Full Redundancy)

Every component is fully duplicated:
- If you need 4 power circuits, you have 8
- Either half can power the entire system independently
- Used for mission-critical AI inference (payment processing, healthcare)

### Dual Network Paths

```
Standard:  Each server has 1 network connection
AI:        Each DGX H100 has 8× 400Gb InfiniBand connections
           for redundancy and performance

If one NIC fails → traffic routes through the other 7
```
...
**Q: How much power does a DGX H100 draw at maximum load?**
A: 10.2kW (kilowatts). This is equivalent to about 100 hair dryers running at once.

**Q: A standard data centre rack handles 10kW. A team wants to deploy 2 DGX H100 systems. What is the problem?**
A: Two DGX H100 systems draw 20.4kW — more than double the rack's capacity. They need a data centre with high-density power infrastructure and Direct Liquid Cooling.

**Q: What is Direct Liquid Cooling (DLC) and why is it required for modern AI systems?**
A: DLC pipes cold water directly to GPU cooling plates. Required because air cooling is limited to ~20kW per rack, while high-density AI systems need 30–100kW per rack.

**Q: What is the difference between N+1 and 2N redundancy?**
A: N+1 = one spare component for the full load. 2N = full duplication — either half can independently handle the entire load. 2N is used for mission-critical applications.

**Q: What is the form factor of a DGX H100?**
A: 6U (six rack units). A standard rack is 42U, so a fully populated rack with DGX H100s holds about 7 systems (7 × 6U = 42U).

---

---

# S-NV-2: Base Command Manager

## What Base Command Manager Is

⚡ **Tested on NCA-AIIO and sometimes NCA-AIIO practice exams use it for cluster management questions.**

**NVIDIA Base Command Manager (BCM)** is NVIDIA's software for managing AI computing clusters — groups of GPU servers that work together.

> 🏗️ **Building manager analogy:** If a DGX H100 is an apartment unit, Base Command Manager is the building management system — it tracks which units exist, assigns them to tenants, manages the common infrastructure (networking, storage), and handles maintenance.

---

## What Problems BCM Solves

Without cluster management software, running a multi-node AI cluster is chaotic:

```
WITHOUT BCM:
  - SSH into each node manually to check status
  - Manually install software on every node separately
  - No centralised view of what jobs are running where
  - Storage mounted differently on each node
  - Difficult to onboard new nodes

WITH BCM:
  - Single dashboard shows all nodes and their status
  - Software deployed once → pushed to all nodes automatically
  - Job queuing and scheduling handled centrally
  - Shared storage configured once, available everywhere
...
BCM can automatically install the operating system, CUDA drivers, and NVIDIA software stack on new nodes:

```bash
# BCM command to provision a new node
cmsh
# (cmsh = cluster management shell, BCM's command-line interface)

> device open node001          # Open node configuration
> set hostname gpu-node-01     # Set the node's name
> set mac 00:1A:2B:3C:4D:5E   # Set MAC address (hardware identifier)
> commit                        # Save configuration
> exit

# BCM then automatically:
# 1. Detects the new physical server
# 2. Boots it from the network
# 3. Installs the OS
# 4. Installs CUDA, drivers, container toolkit
# 5. Joins it to the cluster
```
...
# Output example:
# node001  UP    8× H100   GPU:OK  Network:OK  Storage:OK
# node002  UP    8× H100   GPU:OK  Network:OK  Storage:OK
# node003  DOWN  8× A100   GPU:FAIL (GPU 3 error)  Network:OK

# Get detailed GPU info for a specific node
> device open node001
> show gpu
```

### 3 — User and Job Management

BCM integrates with Slurm (the job scheduler) to manage who can run what:

```bash
# Add a user to the cluster
cmsh
> user add alice
> user set alice group ml-team
> user set alice homedirectory /home/alice
> commit

# Check what jobs are running
> job list
# Shows all jobs: who submitted them, which nodes they run on, GPU allocation
```

### 4 — Storage Management

```bash
# BCM manages shared storage that all nodes access
cmsh
> filesystem add /data/training
> filesystem set /data/training type nfs
> filesystem set /data/training server storage01
> filesystem set /data/training export /exports/training-data
> commit
# Now all nodes can access /data/training
```

### 5 — Software Image Management

Instead of installing software on each node manually, BCM manages software images:

```bash
# Create a software image with all required packages
cmsh
> softwareimage create ai-training-v2
> softwareimage set ai-training-v2 baseimage ubuntu22.04
# BCM installs: CUDA 12.0, PyTorch 2.1, NCCL, etc. on this image
> commit

# Deploy the image to all nodes
> device set node001 softwareimage ai-training-v2
> device set node002 softwareimage ai-training-v2
> commit
# Both nodes now have identical software environments
```

---

## BCM vs Kubernetes — When to Use Which

⚡ **The exam distinguishes between BCM and Kubernetes.**

| | BCM | Kubernetes |
|---|---|---|
| Primary use | Bare-metal HPC cluster management | Container orchestration |
| Best for | Training (batch jobs, bare metal) | Inference (serving, autoscaling) |
| Job model | Submit batch jobs via Slurm | Deploy pods/services |
| Node provisioning | Yes (automated) | Manual or node pools |
| OS management | Yes (full OS lifecycle) | No (manages containers, not OS) |
| Learning curve | HPC-focused | Cloud-native focused |
| Common with | DGX systems, HPC clusters | Cloud AI, inference serving |

---

## S-NV-2 Flashcards

**Q: What is NVIDIA Base Command Manager?**
A: Cluster management software for AI computing infrastructure. Handles node provisioning, health monitoring, user management, shared storage, and software image deployment across multi-node GPU clusters.

**Q: What is `cmsh`?**
A: The cluster management shell — BCM's command-line interface for managing cluster nodes, users, jobs, and configuration.

**Q: BCM vs Kubernetes — which do you use for training clusters?**
A: BCM for bare-metal training clusters (works with Slurm for job scheduling). Kubernetes for containerised workloads and inference serving.

**Q: What problem does BCM's software image management solve?**
A: Ensures all nodes in a cluster have identical software environments. Instead of installing CUDA, PyTorch, etc. on each node separately, you create one image and deploy it to all nodes consistently.

---

---

# S-NV-3: NVIDIA Nsight — Profiling and Debugging

## What Profiling Is — Plain English

When a training job is slow, the question is: **why?** Is the GPU not being used efficiently? Is data loading too slow? Is a specific layer in the neural network a bottleneck?

**Profiling** means recording exactly what your program does and how long each part takes — like a very detailed time log of every operation.

> ⏱️ **Restaurant kitchen analogy:** A restaurant owner wants to speed up service. Instead of guessing, they time every step: how long each dish takes to prepare, how long the wait for the oven is, when waiters are idle. That time log tells them exactly where the bottleneck is. Profiling does this for GPU programs.

---

## The Two Main NVIDIA Profiling Tools

### 1 — Nsight Systems (nsys)

⚡ **The exam tests Nsight Systems for system-level profiling.**

`nsys` shows you a **timeline** of everything happening: CPU activity, GPU activity, data transfers between CPU and GPU, CUDA kernels executing.

```bash
# Profile a training script with Nsight Systems
nsys profile python train.py

# This produces a .nsys-rep file you can view in Nsight Systems GUI

# Or get a quick text summary
nsys profile --stats=true python train.py

# Key things to look for in the output:
# - GPU utilisation: is the GPU busy or idle?
# - Memory transfers: how much time is spent moving data CPU→GPU?
# - CUDA kernel time: which operations take the longest?
# - CPU/GPU overlap: is the CPU preparing the next batch while GPU runs?
```

**What a good vs bad profile looks like:**

```
GOOD PROFILE — GPU is always busy:
Timeline:
GPU: [KERNEL][KERNEL][KERNEL][KERNEL][KERNEL]
CPU: [DATA PREP][DATA PREP][DATA PREP][DATA PREP]
     ↑ CPU and GPU work simultaneously (overlapped)

BAD PROFILE — GPU waits for CPU data loading:
Timeline:
GPU: [KERNEL][IDLE ][IDLE ][IDLE ][KERNEL][IDLE ]
CPU: [DATA PREP  →  →  →  done][DATA PREP  →  →]
     ↑ GPU idle = data starvation = wasted money
```

### 2 — Nsight Compute (ncu)

More detailed than nsys. Shows you metrics for specific CUDA kernels:
- Memory bandwidth utilisation
- Compute throughput
- Which bottleneck: memory-bound vs compute-bound

```bash
# Profile a specific kernel in detail
ncu --target-processes all python train.py

# Useful flags:
ncu --metrics gpu__time_active,sm__throughput.avg.pct_of_peak python train.py
```
...
|---------|--------------|-----|
| GPU idle gaps between kernels | CPU is too slow preparing next batch | Increase DataLoader workers, use prefetching |
| Low SM utilisation | Batch size too small | Increase batch size |
| Memory bandwidth at 90%+ | Memory-bound operation | Use smaller precision (FP16), increase batch size |
| PCIe transfers taking 40%+ of time | Too much CPU→GPU copying | Use pinned memory, overlap transfers with compute |
| One layer takes 80% of runtime | Bottleneck kernel | Focus optimisation there, consider fusion |

---

## Quick Reference — nvidia-smi Profiling

For quick checks without installing Nsight:

```bash
# Watch GPU utilisation in real time
nvidia-smi dmon -s u

# Output (refreshes every second):
# # gpu   pwr  gtemp  mtemp    sm   mem   enc   dec  mclk  pclk
#    0   350W    72C     -    87%   45%    0%    0%  1593  1410
#    1   355W    71C     -    92%   48%    0%    0%  1593  1410
# sm% = how busy the GPU cores are (want this HIGH, ideally 80%+)
# mem% = memory bandwidth utilisation
...
A: Nsight Systems (nsys) gives a system-level timeline view — CPU activity, GPU activity, data transfers, overall efficiency. Nsight Compute (ncu) gives deep kernel-level metrics for specific CUDA operations.

**Q: On an Nsight Systems timeline, you see the GPU is idle 60% of the time. What does this indicate?**
A: Data pipeline bottleneck (data starvation). The GPU finishes processing one batch and sits idle waiting for the next batch to be loaded from storage. Fix: increase DataLoader workers, use data prefetching, or use faster storage.

**Q: What command runs a quick GPU utilisation monitoring dashboard?**
A: `nvidia-smi dmon -s u` — shows SM utilisation, memory bandwidth, power, and temperature for all GPUs, updated every second.

**Q: What does low SM utilisation combined with high memory bandwidth utilisation indicate?**
A: The workload is memory-bound — limited by how fast data can be moved, not by compute. Fix: use lower precision (FP16/INT8) to reduce data size, increase batch size for better memory access patterns.

---

---

# S-NV-4: GPUDirect Storage and RoCE — Expanded

## GPUDirect Storage — Deep Dive

### The Problem It Solves

When training a neural network, data must travel from storage (hard drive or NVMe SSD) all the way to GPU memory:

```
WITHOUT GPUDirect Storage:
  NVMe SSD
      ↓ (PCIe)
  System RAM (CPU copies data here first)
      ↓ (CPU does data processing/decoding)
  System RAM (processed data sits here)
      ↓ (PCIe, another transfer)
  GPU Memory (VRAM)

Problems:
  - Data is copied TWICE (storage → RAM, RAM → GPU)
  - CPU is busy copying data instead of doing useful work
  - High latency from two transfers
```
...
### When to Use GPUDirect Storage

| Use GPUDirect Storage | Standard path is fine |
|-----------------------|-----------------------|
| Training with very large datasets | Small datasets that fit in RAM |
| Data loading is your bottleneck | Compute is your bottleneck |
| NVMe drives directly attached to server | Cloud object storage (S3) |
| Maximum throughput needed | Standard throughput acceptable |

### GPUDirect Storage Requirements

```
Hardware required:
  - NVIDIA GPU (Ampere/A100 or newer)
  - NVMe SSD (directly attached via PCIe)
  - Supported operating system (Linux)

Software required:
  - CUDA 11.4 or later
  - cuFile API (NVIDIA's library for GPUDirect Storage)
...
### What RDMA Is (Reminder)

RDMA = Remote Direct Memory Access. One machine's GPU can directly write into another machine's GPU memory over the network, without either CPU getting involved.

### What RoCE Is

**RoCE (RDMA over Converged Ethernet)** brings RDMA capability to standard Ethernet networks. Instead of needing specialised InfiniBand hardware, you can get RDMA performance on Ethernet.

> 🛣️ **Road analogy:** InfiniBand is a private, purpose-built highway with no traffic — extremely fast but expensive. Standard Ethernet is a regular road with all the stops and traffic lights. RoCE is like adding express lanes and traffic signal priority to a regular road — not quite as fast as the private highway, but much less expensive and works with existing infrastructure.

### RoCE v1 vs RoCE v2

| | RoCE v1 | RoCE v2 |
|---|---|---|
| Network layer | Layer 2 (Ethernet only, same switch) | Layer 3 (IP-routable across switches) |
| Scalability | Limited to one Ethernet broadcast domain | Can span multiple switches and subnets |
| Use in production | Legacy, limited | Current standard |
| Requires | Lossless Ethernet (priority flow control) | Lossless Ethernet + IP routing |

⚡ **Exam point:** RoCE v2 is the current standard because it can be routed across network switches, making it practical for large clusters.

### Lossless Ethernet — Why It Matters for RoCE

Standard Ethernet drops packets when a switch is congested. RDMA operations cannot handle dropped packets — retransmitting breaks the "zero copy" guarantee.

For RoCE to work, the network must be configured as **lossless**:

```
Priority Flow Control (PFC):
  When a switch buffer fills up, it sends a PAUSE frame
  The sending port stops sending temporarily
  No packets are dropped
  RDMA operations complete successfully

Data Center Bridging (DCB):
  The full suite of standards that makes Ethernet lossless
  Required for RoCE to function correctly
```

### InfiniBand vs RoCE — Decision Guide

⚡ **NCA-AIIO exam decision table:**

| Factor | Choose InfiniBand | Choose RoCE |
|--------|-----------------|-------------|
| Latency requirement | Ultra-low (<2μs) | Moderate (2–10μs) |
| Scale | 1000+ GPUs | Up to ~500 GPUs |
| Budget | Higher | Lower (reuse Ethernet) |
| Network team expertise | HPC background | Ethernet background |
| New installation | Either works | RoCE simpler with existing Ethernet |
| NVIDIA recommendation | For largest training clusters | For most inference, moderate training |

---

## S-NV-4 Flashcards

**Q: What is the key performance advantage of GPUDirect Storage?**
A: Data travels directly from NVMe SSD to GPU memory in one transfer, bypassing system RAM. Eliminates one copy operation, frees the CPU, and reduces latency.

**Q: What is RoCE and why does it exist?**
A: RDMA over Converged Ethernet — brings RDMA (direct memory access between GPU nodes) to standard Ethernet networks. Exists because InfiniBand is expensive; RoCE provides similar benefits on existing Ethernet infrastructure at lower cost.

**Q: What is lossless Ethernet and why do you need it for RoCE?**
A: Ethernet configured to never drop packets (using Priority Flow Control). Required for RoCE because RDMA operations cannot handle packet loss — dropped packets break the direct memory transfer.

**Q: RoCE v1 vs RoCE v2 — what is the key difference?**
A: RoCE v1 only works within one Layer 2 Ethernet domain (same switch). RoCE v2 uses Layer 3 IP routing and can span multiple switches — making it practical for real production clusters.

---

---

# S-NV-5: vGPU — Virtual GPU Technology

## What vGPU Is — Plain English

Normally, a GPU is used by one job at a time. Either one user's training job runs on it, or one inference server runs on it.

**vGPU (Virtual GPU)** is NVIDIA's technology to share one physical GPU among multiple virtual machines — like multiple renters sharing one apartment, with thin walls that keep their activities separate.

> 🏢 **Apartment vs hotel analogy:**
> - No vGPU = one family owns the entire house (one workload per GPU)
> - vGPU = apartment building (multiple tenants, shared building infrastructure, separate spaces)
> - MIG = condo ownership (each owner has a legally separated unit with no shared walls)

---

## vGPU vs MIG — The Important Difference

⚡ **This comparison is directly tested on NCA-AIIO.**

| Feature | vGPU | MIG (Multi-Instance GPU) |
|---------|------|--------------------------|
| Isolation | Software (virtualisation layer) | Hardware (physically partitioned) |
| Performance overhead | Yes (typically 5–15%) | Near zero (bare metal) |
| Flexibility | Dynamic, resize VM vGPUs | Fixed profiles at partition time |
| Works with VMs | Yes — designed for VMs | No — requires bare metal or containers |
| Security isolation | Good (hypervisor level) | Excellent (hardware level) |
| GPU support | Older and newer GPUs | A100, H100, and newer only |
| Typical use case | Enterprise VDI, mixed workloads | Data centre AI, multi-tenant inference |

### When to Use vGPU

```
✅ Use vGPU when:
  - Multiple users need VM-based GPU access
  - Running a mix of AI and visualisation workloads on same GPU
  - Your organisation is heavily virtualised (VMware, Hyper-V)
  - Need to support legacy applications requiring Windows VMs

❌ Don't use vGPU when:
  - Maximum AI training performance is needed (overhead is unacceptable)
  - Running on A100/H100 for pure ML workload (use MIG instead)
  - Security isolation must be hardware-guaranteed (use MIG)
```

### vGPU Profiles

vGPU divides a GPU into profiles of different sizes:

```
NVIDIA A100 80GB vGPU profiles (examples):

Profile          VMs    Memory    Intended use
A100-10C         8      10GB      Light compute workloads
A100-20C         4      20GB      Moderate compute
A100-40C         2      40GB      Large AI workloads
A100-80C         1      80GB      Full GPU (passthrough)

"C" = Compute optimised (for AI workloads)
"Q" = Quadro (for graphics/visualisation workloads)
```

### NVIDIA AI Enterprise and vGPU

NVIDIA AI Enterprise is the enterprise software suite that includes:
- vGPU software (the virtualisation layer)
- Certified AI frameworks (PyTorch, TensorFlow) validated for vGPU
- Support on VMware vSphere, Red Hat OpenShift, Citrix
- Enterprise support SLAs

⚡ **Exam note:** vGPU requires a NVIDIA AI Enterprise licence. It is not free like CUDA.

---

## S-NV-5 Flashcards

**Q: What is the main difference between vGPU and MIG isolation?**
A: vGPU isolation is at the software/virtualisation layer (some overhead, works with VMs). MIG isolation is hardware-level (near-zero overhead, requires bare metal or containers, A100/H100+ only).

**Q: When would you choose vGPU over MIG?**
A: When running virtualised workloads (VMs), when you need to support Windows VMs, when the organisation is heavily virtualised (VMware/Hyper-V), or when running a mix of AI and visualisation on the same GPU.

**Q: What does the "C" suffix mean in a vGPU profile like "A100-20C"?**
A: Compute-optimised profile, designed for AI and HPC workloads. "Q" profiles are for graphics and visualisation workloads.

**Q: What software product from NVIDIA enables vGPU?**
A: NVIDIA AI Enterprise — the enterprise software suite that includes the vGPU software layer. Requires a paid licence.
...
## CCA — Topics Now Fully Covered

| Added section | Domain | Coverage |
|--------------|--------|---------|
| Agent SDK — what it is, when to use it, full code | D1 (27%) | ✅ Complete |
| Plan mode — `--plan` flag, when to use | D2 (20%) | ✅ Complete |
| /init command — what it does | D2 (20%) | ✅ Complete |
| Custom Agent Skills — creation, triggering, sharing | D2 (20%) | ✅ Complete |
| Confidence calibration — with code | D1 + D5 | ✅ Complete |
| Provenance tracking — with code | D1 + D5 | ✅ Complete |

## NCA-AIIO — Topics Now Fully Covered

| Added section | Domain | Coverage |
|--------------|--------|---------|
| DGX systems specs, power, cooling | A (40%) | ✅ Complete |
| Power calculation scenarios | A (40%) | ✅ Complete |
| Air vs DLC vs immersion cooling | A (40%) | ✅ Complete |
| Base Command Manager — full coverage | A (40%) | ✅ Complete |
| Nsight Systems and Nsight Compute | C (22%) | ✅ Complete |
| Profiling interpretation | C (22%) | ✅ Complete |
| GPUDirect Storage — deep dive | A (40%) | ✅ Complete |
| RoCE v1 vs v2, lossless Ethernet | A (40%) | ✅ Complete |
| vGPU vs MIG comparison | A (40%) | ✅ Complete |
| NVIDIA AI Enterprise + vGPU | A + B | ✅ Complete |

---

## Combined Exam Checklists

### CCA — Complete Domain Checklist

```
DOMAIN 1 — Agentic Architecture (27%)
□ Agent loop: stop_reason end_turn vs tool_use
□ Three stopping conditions: success + error threshold + max iterations
□ Agent SDK: when to use vs raw API, tool dispatcher pattern
□ Task decomposition: break goal before starting
□ Session resumption: save state every iteration
□ Hub-and-spoke: orchestrator + fresh-context sub-agents
□ Guardrails: input (before Claude) + output (after Claude)
□ Hooks: PreToolUse (blocking/validation) PostToolUse (logging)
□ Hooks vs system prompt: safety-critical = hooks, guidance = prompt
□ Confidence calibration: 0-100 score, escalation thresholds
□ Provenance tracking: source_agent + source_tool + timestamp

DOMAIN 2 — Claude Code (20%)
□ CLAUDE.md: cascade rule (root → subdirectory, specific wins)
□ /init: generates CLAUDE.md from repository analysis
□ -p flag: non-interactive CI/CD mode
□ --plan flag: show proposed changes before executing
□ --yes flag: auto-accept (use carefully)
□ PreToolUse / PostToolUse hooks
□ Custom Agent Skills: .claude/skills/, markdown files, auto-triggered
□ Skills are committed to Git for team sharing

DOMAIN 3 — Structured Output (20%)
□ tool_choice specific: only 100% reliable method
□ tool_choice auto: maybe calls tool, not reliable for structured output
□ Few-shot: show examples, beats zero-shot for specific formats
□ Explicit criteria: measurable, specific, not vague
□ Validation retry loop: validate → send error → retry → max retries
□ Nullable fields: "type": ["string", "null"]

DOMAIN 4 — MCP (18%)
□ Tools: execute, side effects, dynamic data
□ Resources: read-only, static/semi-static data
□ Prompts: reusable templates with variables
□ Tool description formula: what + when + input + returns + NOT
□ Validation at MCP server layer — never trust Claude's inputs
□ Transport: HTTP/SSE for production, stdio for development
□ Scoping: restrict which tools available to which agents/contexts

DOMAIN 5 — Context (15%)
□ 200k token window, saturates silently
□ Saturation symptoms: wrong params, ignored instructions, vague output
□ Fix: summarise history, sub-agents with fresh context, trim outputs
□ Lost in the middle: important info at start, explicit headers
□ Structured errors: error_type + message + suggestion
□ Error propagation: structured response up to orchestrator
□ Confidence calibration: low confidence triggers escalation
□ Provenance: every claim cites source agent and tool
```

### NCA-AIIO — Complete Domain Checklist

```
DOMAIN A — AI Infrastructure (40%)
□ H100 vs A100 vs L40S: training vs inference, bandwidth, power
□ MIG: hardware isolation, profiles (1g.10gb to 7g.80gb), vs vGPU
□ vGPU: software isolation, works with VMs, requires AI Enterprise licence
□ NVLink: GPU-to-GPU, 900 GB/s H100, NVSwitch for 8+ GPUs
□ DGX H100: 8× H100, 640GB, 10.2kW, 6U form factor
□ DGX vs HGX: complete server vs GPU module for OEMs
□ Power: DGX H100 = 10.2kW, standard rack = 5-15kW, AI rack = 30-100kW
□ Cooling: air (<20kW), DLC (to 100kW), immersion (200kW+)
□ Triton: dynamic batching, model repository, config.pbtxt
□ TensorRT: layer fusion, FP16/INT8 quantisation, inference engine
□ NGC: container registry, pre-trained models, Helm charts
□ GPU Operator: automates all NVIDIA k8s components
□ NVIDIA Container Toolkit: --gpus all, --shm-size
□ InfiniBand: 1μs latency, native RDMA, large training clusters
□ RoCE v2: RDMA over Ethernet, IP-routable, needs lossless Ethernet
□ GPUDirect RDMA: GPU-to-GPU over InfiniBand, no CPU
□ GPUDirect Storage: NVMe-to-GPU direct, no system RAM copy
□ BlueField DPU: offloads networking/security, DOCA SDK
□ Base Command Manager: cluster provisioning, health, user management
□ Slurm: HPC job scheduler, #SBATCH directives, --gres=gpu

DOMAIN B — Essential AI Knowledge (38%)
□ AI > ML > DL nested subsets
□ Supervised / Unsupervised / Reinforcement: know each + example
□ Training vs inference: different infrastructure, lifetime, compute
□ GPU parallelism: thousands of cores, matrix operations
□ Tensor Cores: matrix-multiply-accumulate, AI-specific hardware
□ CUDA: software platform, bridge between ML frameworks and GPU
□ cuDNN: DL primitives, called automatically by PyTorch/TensorFlow
□ NCCL: multi-GPU collective comms, AllReduce for gradients
□ TensorRT: optimises trained models for inference
□ Precision types: FP32/BF16 training, FP16/INT8 inference

DOMAIN C — AI Operations (22%)
□ DCGM: GPU metrics, dcgmi diag, dcgmi dmon, DCGM Exporter
□ nvidia-smi: dmon, pmon, query flags for monitoring
□ Low GPU util: data starvation (most common)
□ OOM: reduce batch, FP16, gradient checkpointing
□ Thermal throttling: clock speed drops, check temperature
□ NCCL errors: network config, firewall, version mismatch
□ Nsight Systems (nsys): system-level timeline profiling
□ Nsight Compute (ncu): kernel-level detailed profiling
□ Mixed precision: FP16 forward, GradScaler, 2× throughput
□ Data starvation fix: faster storage, more workers, prefetching
```
